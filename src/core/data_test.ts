...
export function registerDataTests(): void {
  Deno.test("data: returns empty object when _data dir does not exist", async () => {
    const tempDir = Deno.makeTempDirSync();
    const result = await loadDataFiles(tempDir);
    assertEquals(result, {});
  });

  Deno.test("data: loads a JSON file", async () => {
    const dir = makeDataDir({ "team.json": `[{"name":"Alice"}]` });
    const result = await loadDataFiles(dir);
    assertEquals(result.team, [{ name: "Alice" }]);
  });

  Deno.test("data: loads a YAML file", async () => {
    const dir = makeDataDir({ "nav.yaml": `- title: Home\n  url: /\n` });
    const result = await loadDataFiles(dir);
    assertEquals(result.nav, [{ title: "Home", url: "/" }]);
  });

  Deno.test("data: loads a TOML file", async () => {
    const dir = makeDataDir({ "site.toml": `title = "My Site"\n` });
    const result = await loadDataFiles(dir);
    assertEquals(result.site, { title: "My Site" });
  });

  Deno.test("data: nested directory becomes nested key", async () => {
  ...
  Deno.test("data: skips unsupported file types", async () => {
    const dir = makeDataDir({
      "notes.txt": `hello`,
      "team.json": `[{"name":"Alice"}]`,
    });
  ...
  Deno.test("data: yml extension is supported", async () => {
    const dir = makeDataDir({ "config.yml": `debug: true\n` });
    const result = await loadDataFiles(dir);
    assertEquals(result.config, { debug: true });
  });

  Deno.test("data: invalid JSON is skipped with warning", async () => {
