...
export function getEnvironmentFilePaths(rootDir: string, environment: StenoEnvironment): string[] {
  return [
    join(rootDir, ".env"),
...
export function parseEnvironmentFile(source: string, filePath = ".env"): Record<string, string> {
  ...
  for (const [index, rawLine] of source.split(/\r?\n/).entries()) {
    ...
    if (separator < 1) {
      throw new Error(`Invalid dotenv entry in "${filePath}" at line ${index + 1}.`);
    }
    const key = normalized.slice(0, separator).trim();
    if (!/^[A-Za-z_][A-Za-z0-9_]*$/.test(key)) {
      throw new Error(`Invalid dotenv key "${key}" in "${filePath}" at line ${index + 1}.`);
    }
    ...
    try {
      ...
      throw new Error(
        `Invalid dotenv value for "${key}" in "${filePath}" at line ${index + 1}: ${errorMessage(
          error,
        )}.`,
      );
    }
...
export function loadEnvironmentFiles(
  ...
  for (const filePath of getEnvironmentFilePaths(rootDir, environment)) {
    try {
      Object.assign(values, parseEnvironmentFile(Deno.readTextFileSync(filePath), filePath));
    } catch (error) {
