...
This report is generated by `deno task bench:report`. Results are evidence from one machine, not
universal guarantees; compare runs made on equivalent hardware and runtime versions.
...
Cold scenarios delete output and persistent cache before every measured build. Warm unchanged uses
an initialized 1,000-page site. Atomic incremental changes one page while retaining transactional
staging and promotion. Fixture creation is outside the explicit benchmark timer.
...
Run on an otherwise idle machine. The performance gate evaluates averages against committed budgets;
this report additionally publishes sample count, minimum, p75, and p99 latency.
