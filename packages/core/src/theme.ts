...
import { resolveSchemaDefaults, type ThemeConfig, validateThemeConfig } from "./theme_config.ts";

export type { ThemeConfig } from "./theme_config.ts";

/** Product-owned locations for themes resolved by directory loaders. */
export type BundledThemeSources = Readonly<Record<string, URL>>;

/** Returns a filesystem path for a product-supplied local theme URL. */
...
export class Theme {
   ...
   * @param bundledThemes - Product-owned directory locations for named base themes.
   * @returns A new {@link Theme} instance.
   */
  public static async loadFromDirectory(
    dir: string,
    userConfig: ThemeConfig = {},
    bundledThemes: BundledThemeSources = {},
  ): Promise<Theme> {
    const { theme, layoutPaths, componentPaths } = await Theme.loadDirectoryChain(
      dir,
      new Set(),
      bundledThemes,
    );

    const themeInstance = new this(theme, userConfig);
   ...
   * `mergeTheme`s the base theme underneath it with the same field-by-field
   * semantics as module-based theme composition. Source-path maps (used for Tau error messages) merge the
  ...
  private static async loadDirectoryChain(
    ...
    const baseDir = Theme.resolveExtendsDir(dir, metadata.extends, bundledThemes);
    const base = await Theme.loadDirectoryChain(baseDir, seen, bundledThemes);
    return {
   ...
   * product-supplied bundled specifiers or a local path. Relative paths
   ...
   */
  private static resolveExtendsDir(
    dir: string,
    specifier: string,
    bundledThemes: BundledThemeSources,
  ): string {
    const bundledSource = bundledThemes[specifier];
    if (bundledSource) {
    ...
    const bundledNames = Object.keys(bundledThemes);
    const available = bundledNames.length > 0 ? ` (${bundledNames.join(", ")})` : "";
    throw new Error(
      `Theme "extends: ${specifier}" is not a recognized bundled theme${available} or a local path ` +
