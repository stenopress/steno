...
export interface MarkdownPage {
  /** Absolute source path and identity used by content caches. */
  fullPath: string;
  /** Source path relative to the content root. */
  relPath: string;
  /** Original document, including frontmatter. */
  sourceText: string;
  /** Parsed YAML or TOML metadata. */
  frontmatter: Record<string, unknown>;
  /** Markdown source after frontmatter removal. */
  body: string;
  /** Title inferred during discovery, if available. */
  title?: string;
}
...
export async function buildCollections(
  ...
  for (const page of markdownPages) {
    ...
    if (htmlContent === undefined) {
      htmlContent = await renderMarkdown(body, plugins);
