...
export { isPathInsideOrEqual } from "./fs.ts";

/** Content metadata used to infer a title and resolve a public route. */
export interface RoutablePage {
  /** Optional absolute source path. */
  fullPath?: string;
  /** Source path relative to the content root. */
  relPath: string;
  /** Optional original document, including frontmatter. */
  sourceText?: string;
  /** Parsed frontmatter, including optional permalink overrides. */
  frontmatter: Record<string, unknown>;
  /** Markdown body used to infer a heading-based title. */
  body: string;
  /** Previously inferred title, when available. */
  title?: string;
}
...
/** Infers a title from frontmatter, the first level-one heading, or the source path. */
